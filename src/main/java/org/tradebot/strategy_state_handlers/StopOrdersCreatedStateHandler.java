...
public class StopOrdersCreatedStateHandler implements StateHandler {

    private final Log log;
    private final OrderManager orderManager;
    private final APIService apiService;

    public StopOrdersCreatedStateHandler(APIService apiService,
                                         OrderManager orderManager,
                                         int clientNumber) {
        this.apiService = apiService;
        this.orderManager = orderManager;
        this.log = new Log(clientNumber);
        log.info("StopOrdersCreatedStateHandler initialized");
    }
    ...
    private boolean validateTwoOrdersState(List<Order> openedOrders) {
        ...
        if (take0 != null) {
            ...
            if (take0Opt.isEmpty()) {
                ...
                log.info("First take order is still opened. Invalid state.");
                return false;
            }
        } else {
