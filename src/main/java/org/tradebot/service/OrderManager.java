...
public class OrderManager {
    ...
    public void placeOpenOrder(final Imbalance imbalance, double price) {
        ...
        double quantity = apiService.getAccountBalance().getSuccessResponse() * leverage / price;
        synchronized (lock) {
            log.info("Placing open order...");
            Order openOrder = apiService.placeOrder(orderUtils.createOpen(symbol, imbalance, quantity)).getSuccessResponse();
    ...
    public void handleOrderUpdate(String clientId) {
        ...
        synchronized (lock) {
            if (clientId.equals(orders.get(OrderType.OPEN))) {
                ...
                log.warn(String.format("Unknown order update received. Client ID: %s", clientId));
            }
        }
    }
    ...
    private void handleOpenOrderFilled() {
        ...
        Position position = apiService.getOpenPosition(symbol).getSuccessResponse();
        if (position == null) {
    ...
    public void placeClosingOrdersAndScheduleCloseTask(Position position) {
        ...
        if (errors.isEmpty()) {
            ...
            scheduleAutoCloseTask(position);
        } else {
    ...
    public void close(Position position) {
        ...
        Order closeOrder = orderUtils.createClosePositionOrder(symbol, position, CLOSE_CLIENT_ID_PREFIX + System.currentTimeMillis());
        closePosition(OrderType.CLOSE, closeOrder);
    }

    protected void closeTimeout(Position position) {
        log.info("Closing position due to timeout...");
        Order timeoutOrder = orderUtils.createClosePositionOrder(symbol, position, TIMEOUT_CLOSE_CLIENT_ID_PREFIX + System.currentTimeMillis());
        closePosition(OrderType.TIMEOUT, timeoutOrder);
    }

    protected void closeForce(Position position) {
        log.warn("Forcefully closing position due to critical error...");
        Order order = orderUtils.createClosePositionOrder(symbol, position, FORCE_CLOSE_CLIENT_ID_PREFIX + System.currentTimeMillis());
    ...
    protected void closePosition(OrderType orderType, Order order) {
        synchronized (lock) {
            ...
            if (response.isSuccess()) {
                ...
                log.info(String.format("Position closed successfully. Order details: %s", response.getValue()));
            } else if (response.getError().code() == -2022) {
                log.warn("Got error 'ReduceOnly Order is rejected' which mean that position is empty");
                resetToEmptyPosition();
            } else {
                log.error("Unexpected error during position closure. Manual intervention may be required.");
            }
        }
    }
